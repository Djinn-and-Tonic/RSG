...
from rsg_version import version_hash_1, version_hash_2, VersionError, check_rando_version
check_rando_version()

# Please set the weights file you with to load
weights = 'rsg' # The default Random Settings Generator weights
# weights = 'full-random' # Every setting with even weights
# weights = 'coop' # Uses the rsg weights with some extra modifications
...
# mw_weights_file = 'rsg_multiworld.json' # If this variable exists, load this file and use it to edit loaded weights
...
RSG_TRICKS = True # Adds specified tricks
RSG_CONDITIONALS = True # Adds specific conditionals
...
def generate_balanced_weights(fname='default_weights.json'):
    ...
    exclude_from_weights = ['bridge_tokens', 'lacs_tokens', 'triforce_goal_per_world', 'disabled_locations',
                            'allowed_tricks', 'starting_equipment', 'starting_items', 'starting_songs', 'text_shuffle']
    weight_dict = {}
...
def add_rsg_tricks(random_settings):
...
def main():
    ...
    remove_me = ['random_settings_generation.json', 'ERRORLOG.TXT']
    ...
    if weights in ['rsg', 'coop']:
        weight_dict = load_weights_file('rsg_weights.json')
    ...
    # fuck text shuffle
    if weights == 'rsg':
        weight_dict['text_shuffle'] = {"none": 100}
    ...
    if RSG_TRICKS:
        add_rsg_tricks(random_settings)
